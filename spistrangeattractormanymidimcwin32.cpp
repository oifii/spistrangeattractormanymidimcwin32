...
// spistrangeattractormanymidimcwin32.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "spistrangeattractormanymidimcwin32.h"
...
TCHAR szTitle[1024]={"spistrangeattractormanymidimcwin32title"};					// The title bar text
TCHAR szWindowClass[1024]={"spistrangeattractormanymidimcwin32class"};			// the main window class name
                     ...
                     int       nCmdShow)
{
	...
	if(nArgs>5)
	{
		global_sleeptimeperstrangeattractor_sec = atof(szArgList[5]);
	}
	if(nArgs>6)
	{
	...
	if(nArgs>18)
	{
		global_idcolor = atoi(szArgList[18]);
	}
	if(nArgs>19)
	{
		strcpy(szWindowClass, szArgList[19]); 
	}
	if(nArgs>20)
	{
		strcpy(szTitle, szArgList[20]); 
	}
	if(nArgs>21)
	{
	...
	{
		if(pFILE) fprintf(pFILE, "invalid noteremapstring, noteremapstring must either be NONE or C_MINORPENTATONIC.\n");
		return FALSE;
	}
	if(global_noteremapstring=="NONE")
	{
	...
	*/
	...
	//LoadString(hInstance, IDC_SPISTRANGEATTRACTORMANYMIDIMCWIN32, szWindowClass, MAX_LOADSTRING);
	...
	{
		hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_SPISTRANGEATTRACTORMANYMIDIMCWIN32));
	}
	else
	{
		hAccelTable = NULL;
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_SPISTRANGEATTRACTORMANYMIDIMCWIN32));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);

	if(global_menubardisplay)
	{
		wcex.lpszMenuName = MAKEINTRESOURCE(IDC_SPISTRANGEATTRACTORMANYMIDIMCWIN32);
	}
	else
	{
